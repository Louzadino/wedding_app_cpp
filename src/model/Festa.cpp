...
Festa::Festa(const string& idFesta, const string& idCasamento, const string& endereco,
    ...
    if (idFesta.empty() || !regex_match(idFesta, regex("\\d{32}"))) {
        throw DataInconsistencyException("O ID da festa deve conter exatamente 32 dígitos numéricos.");
    }
    ...
    if (endereco.empty()) {
        throw DataInconsistencyException("O endereço não pode ser nulo.");
    }
    if (!regex_match(hora, regex("([01]\\d|2[0-3]):[0-5]\\d"))) {
        throw DataInconsistencyException("A hora da festa deve estar no formato HH:mm (00:00 a 23:59).");
    }
    if (numParcelas <= 0) {
...
void Festa::adicionarConvidado(const string& convidado) {
    if (convidado.empty()) {
        throw DataInconsistencyException("O nome do convidado não pode ser vazio.");
    }
...
void Festa::removerConvidado(const string& convidado) {
    ...
    if (it != convidados.end()) {
        ...
        throw DataInconsistencyException("Convidado não encontrado na lista.");
    }
}
